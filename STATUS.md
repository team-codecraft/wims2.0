...
- **2026-04-27 데이터 마이그레이션 미실시 정책 반영 (구두결정 후속):** PO 초기 결정사항(빈 DB 시작·As-Is 참고만·마스터 시드 입력)이 PP/AN/DE 문서에 미반영 상태였음을 발견 → 4개 기획·분석 문서 일괄 개정. **PP00 v1.2→v1.3** (M7·P2-4 마이그→시드) / **PP-SCH v1.0→v1.1** (DE41 비고·Gate 5 진입조건) / **PP12-1 테일러링 v2.1→v2.2** (§1 DE41 적용방안 재기술·IM "데이터이행결과서"→"초기데이터시드입력결과서" 명칭 변경 ●필수 유지·§4 Gate 5) / **AN22-1 v1.0→v1.1** (§5 "현행→To-Be 전환" → "참고용 As-Is 분석·시드 입력 정책" 재기술, §5.3 5건 시드 입력 명세로 교체: 자재 87종·제품 25종·사용자 14명·프로젝트/견적/계약 As-Is 미이관). 외부 wikilink 5개 문서 일괄 치환.
- **2026-04-27 DE34-1·DE41 산출물 작성:** Gate 2 진입조건 충족. **DE34-1 표준 데이터사전 v1.0** 신설(357행 — §2 자재코드 체계 단독 SOT[원자재 UNI-A·부자재 NN-XXXX·반제품 F/HC/HX/-H·위치 H01~W03 10종]·자재 87종·협력업체 11개사·공정 13/12/6·BOM Level 5단 매핑, §3·§4 컬럼·코드 사전 인용 인덱스, §5 Phase 2 [TBD]). **DE41 초기 데이터 시드 입력 정책서 v1.0** 신설(683행 — §2 시드 계획·§3 도메인별 명세[자재·제품 122행 깊이]·§3.1 매트릭스 16행·§4 표준 규칙·§6 검증 정책[합격기준 5건]. 마이그 색채 0건 사용처). [[2026-04-27-de34-de41-design|spec]] / [[2026-04-27-de34-de41-implementation|plan]] 기반 subagent-driven 작성, spec compliance·code quality 양 단계 통과.

## 현재 시점 (2026-04-24)

- **2026-04-27 admin 등록 API 구현 + 시드 admin 추가:** WIMS 2.0 단일 사용자 모델 (사용자 = 관리자) 의 신규 등록을 IF-CM-USR-001 (`POST /api/cm/users`) 본문 구현으로 완성. 신규 service `AdminService.register()` — RSA-OAEP-SHA256 ciphertext 복호화 → DEC-03 10자 정책 → bcrypt(cost=10) 해시 → INSERT, UNIQUE 위반은 409 (`ADMIN_ID_DUPLICATE`/`EMAIL_DUPLICATE`) 매핑. DTO 2종 (`CreateAdminRequest`/`CreateAdminResponse`) + UserController 의 `create()` TODO 본문 채움. **부트스트랩 admin 시드 (Flyway V109)** — `adminId='jinos78'` / `name='관리자'` / 평문 `rhatoRl121!` (10자) 의 BCrypt 해시 사전 계산값 주입. **운영 배포 시 즉시 비밀번호 변경 권고** 명시. 산출물 갱신: **DE24-1 v2.0-r3 → v2.0-r4** (§5.2.3 IF-CM-USR-001 등록 본문 — Request/Response DTO 표·검증 규칙·에러 카탈로그 4종 추가) / **DE33-1 v1.3 → v1.3-r1** (§6.4 신설 V109 admin 시드 SQL + §7.1 V109 행 추가 + 구 V109+ product FK placeholder 를 V110+ 으로 밀어냄) / **AN12-1 P1 v1.1-r4 → v1.1-r5** (FR-CM-004 사용자 등록 수용기준 강화). 후속: 운영 배포 시 시드 admin (`jinos78`) 비밀번호 변경 강제, GlobalExceptionHandler 도입 시 UserController 의 임시 `@ExceptionHandler(AuthException)` 이관.
...
- AN12-1-P1 요구사항정의서 Phase1 **v1.1-r5** — md (**2026-04-27 v1.1-r5 갱신** — FR-CM-004 시스템 관리에 사용자(=관리자) 등록 수용기준 보강. IF-CM-USR-001 (`POST /api/cm/users`) 으로 인증된 admin 만 호출, password 는 IF-CM-AUTH-004 공개키로 RSA-OAEP 암호화 Base64 (평문 금지), adminId/email DB UNIQUE → 409. 첫 admin 은 DE33-1 §6.4 V109 시드로 부트스트랩. FR/NFR 총량 변동 없음). v1.1-r4 (2026-04-27) — FR-CM-008 비밀번호 정책에 전송 구간 RSA-OAEP-SHA256 암호화 수용기준 (수용기준 6·7번) 추가. 평문 password 네트워크 전송 금지 명문화. DE24-1 v2.0-r3 §3.2.6 정합. v1.1-r3 (2026-04-27) — CM 섹션 신규 FR 3건 (FR-CM-007 관리자 인증·FR-CM-008 비밀번호 정책·FR-CM-009 로그인 이력 감사). FR-CM-001 본문 수용기준 강화. CM FR 6→9, Phase 1 합계 70→**73건**. AN12-1 v1.5 md 미러 동시 갱신 (xlsx 본 보류, DEC-01 정책)). v1.1-r2 (2026-04-23) — 분산 구조. v1.1-r1 (2026-04-16) — FR-PM-025/026/027 신설
...
- DE24-1 인터페이스 설계서 **v2.0-r4** (통합본 + IF-CM-USR-001 등록 본문 + 로그인 비밀번호 전송 암호화) — md (**2026-04-27 v2.0-r4 개정** — §5.2.3 IF-CM-USR-001 사용자 등록 본문 명세 보강. `POST /api/cm/users` Request DTO (`CreateAdminRequest` — adminId/password/name/mobile/email) + Response DTO (`CreateAdminResponse`) 표 추가. password 필드는 IF-CM-AUTH-004 공개키 RSA-OAEP-SHA256 암호화 Base64 명시 (평문 금지, 로그인 흐름과 동일). 검증 규칙 (10자 정책·adminId UNIQUE·email UNIQUE) 명문화 + 에러 카탈로그 4종 (`ADMIN_ID_DUPLICATE` 409·`EMAIL_DUPLICATE` 409·`INVALID_PASSWORD_CIPHER` 400·`PASSWORD_POLICY` 400). 첫 admin 부트스트랩은 DE33-1 §6.4 / Flyway V109 시드 마이그레이션에 위임. v2.0 본문 보존, 메타만 r4 표기). v2.0-r3 (2026-04-27, 로그인 password RSA-OAEP-SHA256 암호화) — `/api/auth/login` password 필드 RSA-OAEP-SHA256 암호화 도입. §3.2.6 신설(키 정책·클라이언트 가이드 JS/Kotlin) / §5.2.1 IF-CM-AUTH-001 password 필드 정의 갱신·에러 +2(`INVALID_PASSWORD_CIPHER`/`PASSWORD_POLICY`) / IF-CM-AUTH-004 (`GET /api/auth/public-key`) 신설 / §8 IF-ID 매트릭스 +1 행. v2.0 본문 보존, 메타만 r3 표기). v2.0-r2 (2026-04-27, admin 인증 강화) — §3.2.1 JWT HS256 access 1h / refresh 24h 명문화 + claims 5필드 표준화 + `JWT_SECRET` env var 강제 (prod) / dev fallback 명시. §3.2.4 admin 단일 모델 claims 정정. §3.2.5 Phase 1 ROLE 분기 미적용 정책 명시. §5.2.1 IF-CM-AUTH-001/002/003 본문 전면 강화 — Redis (`refresh:{adminId}:{jti}` / `session:{adminId}` / `bl:{jti}`) · 중복 로그인 방지 · 5회 실패 SUSPENDED 자동 전환 + IF-EVT-USR-001 발행 · LoginRequest/Response·RefreshRequest/Response DTO 표 추가. DE33-1 wikilink v1.2→v1.3 일괄 치환. v2.0 본문 보존, 메타만 r2 표기). v2.0-r1 (2026-04-24) — DEC-06/07 ROLE 사전·BOM_RULE 이력. v2.0 (2026-04-23) — MES 전용→통합본 전면 개편
...
- DE33-1 DB 물리 스키마 설계서 **v1.3-r1** — md (**2026-04-27 v1.3-r1 개정** — 부트스트랩 admin 시드 신설. §6.4 신설 — Flyway V109 `V109__seed_admin.sql` (`adminId='jinos78'` / `name='관리자'` / `status='JOINED'` / password 는 평문 `rhatoRl121!` 의 BCryptPasswordEncoder cost=10 prefix `$2a$` 해시 사전 계산값). 운영 배포 시 즉시 비밀번호 변경 권고 명시. §7.1 V109 행 추가, 구 V109+ product_dies_book_fk 보류 placeholder 를 V110+ 으로 밀어냄. §7.2 의존성 mermaid 갱신. v1.3 본문 보존, 메타만 r1 표기). v1.3 (2026-04-27) — CM 인증 도메인 2 엔티티 신설. §3.21 `admin` (관리자 = 일반 사용자 단일 모델, IF-CM-AUTH-001/002/003 백엔드 SOT, status JOINED/SUSPENDED/WITHDRAWN, bcrypt 해시) + §3.22 `admin_login_history` (로그인 이력 감사, IPv6 대응, 자체 감사 테이블이라 created_at만). Flyway V107·V108 추가. 엔티티 23→**25**, FK 31→32 (Phase 1 활성 29→**30**), 인덱스 48→**52**. v1.2 파일은 git mv 로 v1.3 으로 rename 됨. 개방이슈 #11 (admin_login_history 파티셔닝), #12 (admin↔wims_user Phase 2 정규화) 추가). v1.2 (2026-04-22) — Flyway V100~V106 확정·project_favorite 신설
...
- DE24-1 인터페이스 설계서 **v2.0-r4** (통합본 + admin 인증 + 로그인 비밀번호 전송 암호화 + admin 등록 본문, 2,500+ 라인)
- DE33-1 DB 물리 스키마 설계서 **v1.3-r1** (25 엔티티, Flyway V100~V109 확정 — admin 인증 + 시드 admin 포함)
- DE34-1 표준 데이터사전 **v1.0** (2026-04-27 신설, 357행 — 자재코드 체계 단독 SOT)
- DE41 초기 데이터 시드 입력 정책서 **v1.0** (2026-04-27 신설, 683행 — 마이그 미실시·시드 입력 정책)
...
- **2026-04-27 admin 등록 + 시드 admin 후속:**
  - **운영 배포 시 시드 admin (`jinos78`) 비밀번호 강제 변경** — V109 시드는 부트스트랩 전용. 운영 환경에 V109 적용 직후 IF-CM-AUTH-001 로그인 → IF-CM-USR-002 (Phase 1.5 비밀번호 변경) 또는 별도 운영 admin 1건 이상 생성 후 시드 계정 비밀번호 회전 또는 `status='WITHDRAWN'` 처리. 배포 체크리스트에 등재 필수.
  - **GlobalExceptionHandler 도입** — 현재 `UserController` 에 `@ExceptionHandler(AuthException::class)` 임시 추가. `AuthController` 도 동일 패턴. 전역 핸들러 도입 시 양쪽 컨트롤러의 핸들러를 이관·중복 제거.
  - **사용자 관리 나머지 엔드포인트 구현** — list/get/update/toggleActive/changeRole/resetPassword/unlock 은 본 PR 범위 외 (TODO 유지). FR-CM-004 후속 PR 에서 처리.
