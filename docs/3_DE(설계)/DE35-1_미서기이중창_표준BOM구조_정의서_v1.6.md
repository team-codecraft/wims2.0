---
title: DE35-1 미서기이중창 표준 BOM 구조 정의서 v1.6
doc_code: DE35-1
version: 1.6
created: 2026-03-05
updated: 2026-04-24
...
# DE35-1 미서기이중창 표준 BOM 구조 정의서 v1.6

> [!abstract] 요약
> - 본 문서는 **미서기 이중창(Sliding Double Window)** 의 표준 BOM 구조를 정의하는 **단독 SOT(Single Source of Truth)**. 유니크시스템 현행 BOM 데이터(DHS-AE225-D-1) 와 [[WIMS_용어사전_BOM_v1.4]] 을 기준으로 한다.
> - **v1.6 핵심 확장** (v1.5 대비): 유니크시스템 BOM 2026-04-24 보완본 ([[7_유니크시스템BOM_26.04.24_분석]]) 반영
>   1. **BOM Level 5단 공식 정의** — L1 FERT / L2 HALB / L3 Sub-Asm / L4 Part / L5 ROH (§3.2.1). 유니크시스템 `Sheet1` 이 원천
>   2. **위치구분 10종 공식 카탈로그** — H01~H03-2 7종 + W01~W03 3종, 품목별 사용 패턴·빈도 (§6.4). Q16 해소
>   3. **제품 카탈로그 25종** (후렘 18 + 문짝 6 + 방충망 1) — modelCode 실데이터 매트릭스 (§4A)
>   4. **공정 순서 표준** — 후렘 13공정 / 문짝 12공정 / 방충망 6공정 (§7A). Q19(a) 해소
> - **v1.5 승계 사항** (v1.4 대비):
>   1. PRODUCT/ITEM/OPTION_VALUE/MBOM/BOM_RULE/RESOLVED_BOM 엔티티의 신규 컬럼 11종 + enum 확장 (§3.6, §9)
>   2. 4계층 제품분류(`productClassPath`, `class_l1~l4`) 를 BOM Level 체계와 통합 (§3.2)
>   3. 절단 속성을 MBOM 행 속성으로 정의 — 별도 절단BOM 엔티티 없음 (§3.6)
>   4. BOM_RULE 액션 동사 4종(`SET`/`REPLACE`/`ADD`/`REMOVE`) 스키마 + `rule_type` 도입 (§6.5)
>   5. DHS-AE225-D-1 기준 케이스 실제 DB 샘플 (§10)
>   6. 다이스북·공급망 엔티티 3종(DIES_BOOK/DIES_SUPPLIER/ITEM_SUPPLIER) (§9.7)
>   7. 금지어(`CuttingBOM`·`산식구분`·`계산식`·`공식` 등) 전면 치환 (§12)
> - **SOT 원칙:** 표기 충돌 시 [[WIMS_용어사전_BOM_v1.4]] 우선. BOM Level 체계·위치구분·제품 카탈로그·공정 순서는 **유니크시스템 2026-04-24 보완본** 을 원천으로 한다.

> [!warning] v1.5 대비 변경 영향
> - §3.2.1 **BOM Level 체계를 4단(L0~L3) → 5단(L1~L5) 으로 개정** — FERT/HALB/Sub-Asm/Part/ROH 공식 명명 승격. 유니크시스템 Sheet1 이 원천
> - §4A **제품 카탈로그 25종 표 신설** — modelCode 실데이터, 계열×등급×형식 매트릭스
> - §6.4 **위치구분 10종 카탈로그 공식화** — H01, H02, H02-1, H02-2, H03, H03-1, H03-2, W01, W02, W03 + 품목별 사용 패턴. Q16 해소(미확정 콜아웃 제거)
> - §7A **공정 순서 표준 절 신설** — 후렘 13 / 문짝 12 / 방충망 6 공정. Q19(a) 해소
> - §12 변경이력 v1.6 행 추가
...
1. 개요
2. 용어 범례
3. BOM 구조 개요 (BOM Level 5단 · 제품분류 4계층 · Phantom · 자재분류 · 절단 속성)
4. 미서기 이중창 제품 카탈로그 + Level 전개표
    - §4A 제품 카탈로그 25종 (후렘 18 + 문짝 6 + 방충망 1)
5. BOM 구성 상세 설명
6. BOM 등록 규칙 (품목코드 · 표준BOM 버전 · 수량 · **위치구분 10종 카탈로그** · 룰 스키마)
7. MES 연동 고려사항
    - §7A **공정 순서 표준** (후렘 13 / 문짝 12 / 방충망 6)
8. RuleEngine · 평가 파이프라인
9. 엔티티 스키마 요약 (v1.4 기준)
...
| 문서 코드 | 문서명 | 비고 |
|---|---|---|
| [[WIMS_용어사전_BOM_v1.4]] | BOM 도메인 용어사전 v1.4 | **최상위 기준** |
| [[7_유니크시스템BOM_26.04.24_분석]] | 유니크시스템 BOM 2026-04-24 보완본 분석 | **v1.6 원천** — Level 5단·위치구분 10종·제품 카탈로그 25종·공정 순서 |
| [[DE11-1_소프트웨어_아키텍처_설계서_v1.2]] | SW 아키텍처 설계서 | ER 보강본·frozen 트리거 T1/T2/T3 |
| [[DE32-1_BOM도메인_ER다이어그램_v1.2]] | 논리/물리 ERD | 물리 스키마 확정 |
| [[DE24-1_인터페이스설계서_v2.0]] | MES 인터페이스 설계 | `/bom/resolved/{resolvedBomId}` |
...
본 문서에서 사용되는 전문 용어의 정의는 [[WIMS_용어사전_BOM_v1.4]] 을 최상위 기준으로 한다. 아래에는 미서기 이중창 도메인에 한정된 핵심 용어만 정리한다. (용어사전에 존재하는 항목은 참조 위치를 표기.)
...
복층유리(Pair Glass/IGU), 로이유리(Low-E), 간봉(Spacer Bar), 웜엣지(Warm Edge), 중공층, 아르곤가스, 실란트, 모헤어, 가스켓(EPDM), 단열재(Thermal Break) 는 v1.4 와 동일. 상세 설명은 v1.4 §2.2~2.4 를 본 v1.6 에 승계(동일 텍스트 생략).

### 2.3 시스템·제조 용어

| 용어 | 용어사전 참조 | 정의(요약) |
|---|---|---|
| BOM | v1.4 §2 | EBOM+MBOM+Config 묶음 |
| standardBomId | v1.4 §1 | 표준BOM 영속 식별자. 예: `DHS-AE225-D-1` |
| standardBomVersion | v1.4 §1 | EBOM+MBOM+Config 묶음 단일 버전 카운터 |
| resolvedBomId | v1.4 §1 | `RBOM-{standardBomId}-sbv{N}-{optionsHash}` |
| seriesCode | v1.4 §10 | `PRODUCT.series_code` — 예: `225-마스` |
| productClassPath | v1.4 §9 | `미서기/마스/복층/225` 형태의 4계층 분류 경로 |
| itemCategory | v1.4 §1 | PROFILE/GLASS/HARDWARE/CONSUMABLE/SEALANT/SCREEN |
| cutDirection | v1.4 §3 | `W`/`H`/`W1`/`H1`/`H2`/`H3` |
| supplyDivision | v1.4 §3 | `공통`/`외창`/`내창` |
| OPT-DIM | v1.4 §11 | NUMERIC 치수 옵션 그룹 |
| UNIQUE_V1 | v1.4 §13 | 산식 언어 식별자 |
| Phantom | v1.4 §3 | 재고 미보유 가상 반제품 노드 |
...
본 문서는 **BOM Level 체계(L1~L5, 5단)** 와 **제품분류 4계층(classL1~L4)** 을 **독립적 축**으로 사용한다. Level 은 자재 분해 깊이, Class 는 제품 카탈로그 위치다.

#### 3.2.1 BOM Level 체계 (5단 표준 · Gain A 반영)

v1.6 에서 BOM Level 체계를 **유니크시스템 `Sheet1` 공식 정의** 에 맞춰 5단(L1~L5) 으로 정식화한다. SAP(FERT/HALB/ROH) 네이밍과 일치하며, 유니크시스템 `Sheet1` 이 원천이다 ([[7_유니크시스템BOM_26.04.24_분석]] §4 Gain A).

| Level | 명칭 | SAP 대응 | 설명 | 미서기 이중창 기준 예 |
|---|---|---|---|---|
| **L1** | 완제품 (Finished Goods) | **FERT** | 공장 출하 최종 단위 | `DHS-AE225-D-1` 미서기 이중창 1SET |
| **L2** | 반제품 / 모듈 (Semi-finished) | **HALB** | 독립적 입출고·재고 가능한 조립 모듈 | 창틀 ASSY, 외측 창짝 ASSY, 내측 창짝 ASSY, 방충망 ASSY |
| **L3** | 서브조립 (Sub-Assembly) | — | 조립 공정 중간 노드. 재고 미보유(=Phantom 가능) | 레일 SET, 호차 SET, 크리센트 잠금 SET |
| **L4** | 부품 (Part / Component) | — | 개별 부품 단위 | 상틀, 하틀, 좌틀, 우틀, 복층유리, 호차, 크리센트, 모헤어 |
| **L5** | 원자재 (Raw Material) | **ROH** | 가장 기초 소재 | 프로파일 압출재, 판유리 원판, 간봉, 아르곤가스, 스테인리스 피스 |

> [!info] v1.5 → v1.6 Level 개정
> v1.5 의 4단 체계(L0 완제품 / L1 조립체 / L2 부품 / L3 원자재) 는 **폐기**. 본 문서 §4 Level 전개표의 `Lv` 숫자 컬럼 및 §10 MBOM 샘플의 `level` 컬럼은 **L1~L5 5단 체계** 로 해석한다 (구 0→L1, 1→L2, 2→L4, 3→L5 로 대응. L3 Sub-Asm 은 Phantom 노드로 선택적 삽입).

> [!tip] 구 Level vs 신 Level 매핑 (읽기 가이드)
> | v1.5 구 Lv | v1.6 신 Level | 비고 |
> |---|---|---|
> | 0 (완제품) | **L1** FERT | `PRD-SLD-DW` |
> | 1 (조립체) | **L2** HALB | `ASY-*` 조립체. 재고 보유 시 HALB, 없으면 Sub-Asm(L3)|
> | 2 (부품) | **L4** Part | `FRM-*`, `GLS-*`, `HDW-*` 등 |
> | 3 (소재) | **L5** ROH | `판유리 원판`, `간봉`, `가스` |
> (Phantom 노드 HC-001/HC-002/HX-0001 은 L2 또는 L3 에 배치되며 `is_phantom=TRUE` 플래그로 재고 제외)

> ※ 유니크시스템 원본 BOM 의 "Level 2 반제품(검사)" 는 §3.4 **Phantom 노드** 로 매핑된다. 다만 [[7_유니크시스템BOM_26.04.24_분석]] §3 Q9 회신 결과 `HC-001`·`HC-002`·`HX-0001` 은 실체 반제품(자재코드 부여)로 판명 — Phantom 여부는 `is_phantom` 플래그로 최종 확정한다.

#### 3.2.2 제품분류 4계층 (productClassPath)

(용어사전 v1.4 §9 대응) PRODUCT 행은 `class_l1~l4` 4개 컬럼으로 카탈로그 위치를 갖는다.
...
> [!success] Q9 해소 (2026-04-24, [[7_유니크시스템BOM_26.04.24_분석]] §3)
> `HC-001`·`HC-002`(가공 반제품)·`HX-0001`(조립 반제품) 모두 유니크시스템 신 BOM 에서 **자재코드 부여 확인** → **실체 반제품**이며 Phantom 아님. WIMS 에서 `is_phantom=FALSE` 로 등록하고 독립 재고/공정 노드로 취급한다. Q15 반제품 명명 패턴도 동시 해소(HC-* 가공 / HX-* 조립 / -H 접미사 널링).

### 3.5 자재 분류 체계 + itemCategory

자재는 6가지 `itemCategory` enum 중 하나로 분류된다 ([[WIMS_용어사전_BOM_v1.4]] §1). 이 enum 은 Resolved 단계에서 `actualQty`·`actualCutLength` 산출 산식을 분기하는 키다.
...
v1.4 까지는 "절단 BOM" 이라는 별도 개념이 있었으나, [[WIMS_용어사전_BOM_v1.4]] §12 에 따라 **CuttingBOM 엔티티 폐기** 되었고, 절단 표현은 전부 MBOM 의 행 속성으로 흡수된다.
...
### 4A. 제품 카탈로그 25종 — modelCode 실데이터 (Gain G 반영)

유니크시스템 2026-04-24 보완본 기준 미서기 이중창 modelCode 실데이터 25종 카탈로그. 후렘 18 + 문짝 6 + 방충망 1 = 25종이며, 본 문서의 Level 전개표(§4) 와 BOM_RULE(§6.5) 는 이 카탈로그의 modelCode 를 분기 키(`PRODUCT.model_code`, `series_code`, `class_l2~4`) 로 사용한다.

#### 4A.1 후렘 18종 — 225 × 2등급 × 3형식 + 160 × 3등급 × 3형식 + 140 × 3형식

| # | 치수계열(L4) | 계약구분(L2) | 형식(OPT-LAY) | modelCode 패턴 예 |
|---|---|---|---|---|
| 1 | 225 | 마스 | 1x1 (단창) | `DHS-AE225-D-1` (기본) |
| 2 | 225 | 마스 | 1x2 / 2x1 (중연창) | `225-중연창(1X2)-마스` / `225-중연창(2X1)-마스` |
| 3 | 225 | 마스 | 2x2 (중중연창) | `225-중중연창(2X2)-마스` |
| 4 | 225 | 우수 | 1x1 | `DHS-AE225-U-1x1` |
| 5 | 225 | 우수 | 1x2 / 2x1 | — |
| 6 | 225 | 우수 | 2x2 | — |
| 7 | 160 | 마스(YJN) | 1x1 | `YJN-SL160A-101` 계열 1x1 |
| 8 | 160 | 마스(YJN) | 1x2 / 2x1 | `YJN-SL160A` 중연창 |
| 9 | 160 | 마스(YJN) | 2x2 | `YJN-SL160A` 중중연창 |
| 10 | 160 | 마스(UNI) | 1x1 | `UNI-A160-101` 계열 1x1 |
| 11 | 160 | 마스(UNI) | 1x2 / 2x1 | `UNI-A160-101` 중연창 |
| 12 | 160 | 마스(UNI) | 2x2 | `UNI-A160-101` 중중연창 |
| 13 | 160 | 우수 | 1x1 | — |
| 14 | 160 | 우수 | 1x2 / 2x1 | — |
| 15 | 160 | 우수 | 2x2 | — |
| 16 | 140 | — (없음) | 1x1 | `후렘-140` |
| 17 | 140 | — (없음) | 1x2 / 2x1 | `후렘-140` 중연창 |
| 18 | 140 | — (없음) | 2x2 | `후렘-140` 중중연창 |

> [!note] 160 마스 이원화 (YJN / UNI)
> 160 마스는 `YJN-SL160A-101` 계열과 `UNI-A160-101` 계열 **2개 공급체계가 공존**한다. 공급처·규격 차이는 Q-추가로 확인 예정이며, WIMS 에서는 `PRODUCT.model_code` 로 분리 등록하여 동일 `class_l2='마스'` 하위에 2개 카탈로그 행으로 둔다. `BOM_RULE.when_expr` 에서 `PRODUCT.model_code LIKE 'YJN-%'` / `LIKE 'UNI-%'` 로 분기 가능.

> [!note] 140 등급 차원 NULL 허용
> 140 은 마스/우수 등급 구분이 없다. `PRODUCT.class_l2` 는 **NULL 허용** 컬럼으로 선언되어야 하며, UI(SCR-PM-010) 의 계층 필터에서 L2=등급 선택 시 140 제품은 자동 포함 처리가 필요하다.

#### 4A.2 문짝 6종 — 크리센트 종류 × 치수계열
...
| 4 | 160 | 반달 | — |
| 5 | 160 | 반달세짝문 | — |
| 6 | 140 | 일반 | 크리센트 구분 없이 일반형 1종 |

> 문짝은 크리센트 종류(일자/반달/일반/세짝문) 가 modelCode 세그먼트 후보다. `class_l4` 외에 별도 옵션 그룹 `OPT-CRS`(일자/반달/반달세짝문/일반) 로 모델링할 수 있다.

#### 4A.3 방충망 1종

방충망은 단일 규격 1종으로 관리된다. `ASY-SCR-001` 이 이 카탈로그와 매핑된다.

---

### 4. Level 전개표 (225-마스 DHS-AE225-D-1 기준)

> 아래 표는 미서기 이중창(외측 2짝 + 내측 2짝) 1SET 기준의 표준 BOM 전개다. `Lv` 는 BOM 계층(§3.2.1 **L1~L5 5단 체계** 기준), `수량` 은 바로 위 조립체 1단위 기준 소요량이다. 용어가 낯선 경우 §2 · [[WIMS_용어사전_BOM_v1.4]] 을 참조한다.
>
> 기존 v1.5 의 `Lv` 수치(0/1/2/3) 는 v1.6 L1~L5 로 재해석한다. 표 내부 숫자는 문자 치환 대신 주석으로 안내한다: **Lv 0 → L1, 1 → L2, 2 → L4, 3 → L5**. Sub-Asm(L3) 은 이 전개표에서 Phantom 노드(§3.4) 와 중첩되어 별도 행으로 두지 않는다.
...
`optionsHash` 는 `appliedOptions` 의 **ENUM 옵션 값만** SHA-256 canonical JSON 해시 앞 8자 ([[WIMS_용어사전_BOM_v1.4]] §4.1). NUMERIC 옵션(W/H/W1…) 은 해시 산출에서 제외된다 — `W=1500·H=1200` 과 `W=1501·H=1201` 은 같은 resolvedBomId 로 수렴하고, 실제 치수는 `cut_length_evaluated` 에 별도 snapshot 된다.
...
동결된 `cut_length_evaluated`·`cut_length_evaluated_2`·`cut_qty_evaluated`·`actual_cut_length`·`rule_engine_version` 은 이후 `BOM_RULE.action` 의 산식 상수가 변경되어도, RuleEngine 이 `UNIQUE_V2` 로 업그레이드되어도 **재평가하지 않는다** ([[WIMS_용어사전_BOM_v1.4]] §4.2).
...
### 6.4 위치구분 코드 운용 — 10종 공식 카탈로그 (Gain C 반영)

유니크시스템 2026-04-24 보완본 `위치구분` 컬럼 도입으로 위치구분 카탈로그가 **10종으로 공식 확정**되었다 ([[7_유니크시스템BOM_26.04.24_분석]] §4 Gain C, Q16 해소). `BOM_ITEM_LOCATION` 엔티티로 관리하며, `locationCode` 는 MBOM 행의 `location_code` 컬럼(VARCHAR(8))으로, `variantCode`(예: `UNI-A225-101-HC`) 는 위치 인스턴스 품번으로 저장된다.

#### 6.4.1 위치구분 10종 카탈로그

| locationCode | 계열 | 의미 | 비고 |
|---|---|---|---|
| `H01` | H(수평) | H 1번 위치 | 기본 상/하변. 전 품목 사용 |
| `H02` | H(수평) | H 2번 위치 | 분할 없음 |
| `H02-1` | H(수평) | H02 의 1번 분할 (sub-position) | 중연창·중중연창에서 H02 좌/우 분할 |
| `H02-2` | H(수평) | H02 의 2번 분할 | 중연창·중중연창에서 H02 좌/우 분할 |
| `H03` | H(수평) | H 3번 위치 | 3짝문/3편창 중간 바 |
| `H03-1` | H(수평) | H03 의 1번 분할 | 중중연창(2x2) 에서 H03 좌 |
| `H03-2` | H(수평) | H03 의 2번 분할 | 중중연창(2x2) 에서 H03 우 |
| `W01` | W(수직) | W 1번 위치 | 기본 좌변. 전 품목 사용 |
| `W02` | W(수직) | W 2번 위치 | 후렘·문짝 사용 |
| `W03` | W(수직) | W 3번 위치 | 후렘만 사용 |

**접미사 `-1`/`-2` 규칙**: 동일 위치 내 분할(sub-position). 중중연창(2x2) 에서 H03 좌/우를 분리 표현할 때 사용. 접미사 없는 `H03` 은 분할되지 않은 단일 위치다.

#### 6.4.2 품목별 사용 패턴·빈도 (실데이터 집계)

| 품목 구분 | 사용 locationCode | 품목별 빈도(참고) |
|---|---|---|
| **후렘 (Frame)** | H01, H02, H02-1, H02-2, H03, H03-1, H03-2, W01, W02, W03 (**10종 모두**) | — |
| **문짝 (Sash Door)** | H01, H02, H03, W01, W02, W03 (**6종**) | H02-1/H02-2/H03-1/H03-2 미사용 |
| **방충망 (Screen)** | H01, W01 (**2종**) | 단일 규격 |

| locationCode | 전체 빈도 (유니크시스템 실BOM 집계) |
|---|---|
| H01 | 226회 |
| H02 | 70회 |
| H02-1 | 38회 |
...
- `MBOM.location_code` 는 `BOM_ITEM_LOCATION.location_code` 를 FK 참조. 10종 enum CHECK 제약
- 품목 category 별 허용 locationCode 는 `ITEM_LOCATION_POLICY` 로 별도 관리(후렘=10종 / 문짝=6종 / 방충망=2종)
- 동일 자재가 여러 위치에 사용될 때는 `(item_code, location_code)` 별로 별도 MBOM 행 분리 (§3.6 원칙 승계)

### 6.5 BOM_RULE 액션 동사 스키마 — 4종 verb

(용어사전 v1.4 §13.2 명세화) `BOM_RULE.action_json` 은 JSON 배열. 각 원소는 동사 + 인자로 구성.
...
BOM_RULE (1행):
  ...
  slot_values: <위 slotValues 원본 그대로 저장>
  condition_expr: "productClassPath = '미서기/마스/복층/225' AND OPT-LAY IN ('W3XH2-3편') AND W >= 3000"
  action_json: [
...
BOM_RULE 행 A:
  ...
  action_json: [
    ...
    {verb: SET, target: {itemCode: "PRF-BAR-V"}, field: cutLengthFormula, value: "H - 40"}
  ]
...
## 7A. 공정 순서 표준 (Gain B 반영)

유니크시스템 2026-04-24 보완본 `공정 구분` 시트 기준 품목별 공정 순서 표준. Q19(a) 해소 ([[7_유니크시스템BOM_26.04.24_분석]] §4 Gain B). `MBOM.process_code` 및 `MBOM_PROCESS.sequence_no` 의 기본값·검증 규칙으로 사용한다.

### 7A.1 후렘 13공정 (Frame)

| Seq | 공정명 | 비고 |
|---|---|---|
| 0 | 널링 | 표면 가공. `-H` 접미사 반제품(UNI-A225-10H 등) 생성 |
| 1 | 절단 | `cut_length_formula` 평가값 기반 |
| 2 | 피스홀가공 | 나사 체결 구멍 |
| 3 | 배수홀가공 | 하틀 전용 |
| 4 | 풍지판부착 | 기밀 보강 |
| 5 | 이탈방지장착 | 창짝 이탈방지 |
| 6 | 가네고부착 | 모서리 보강 |
| 7 | 나사체결 | 스테인리스 피스 20EA |
| 8 | 양후렘연결재결합 | 이중창 양측 후렘 연결 |
| 9 | 탄성레일바절단 | 하틀 레일 가공 |
| 10 | 레버부가공 | 크리센트 레버 위치 가공 |
| 11 | 조립 | L2 HALB 로 결합 |
| 12 | 포장 | 출하 |

### 7A.2 문짝 12공정 (Sash Door)
...
| 1 | 절단 | — |
| 2 | 모헤어삽입 | 기밀재 선 조립 |
| 3 | 크리센트부착 | 일자/반달/세짝문 분기 (§4A.2 기준) |
| 4 | 피스홀가공 | — |
| 5 | 코너브라켓결합 | 4모서리 L-브라켓 |
| 6 | 고리캡절단 | 크리센트 고리용 |
| 7 | 고리캡가공 | — |
| 8 | 가이드부착 | 하변 호차 가이드 |
| 9 | 크리센트고리부착 | 외측 우변에 부착 |
| 10 | 조립 | — |
| 11 | 포장 | — |
...
> [!tip] 공정 순서 운용 원칙
> - **후렘은 Seq 0(널링) 이 선택 공정** — `-H` 접미사 원자재 사용 시에만 수행. 일반 원자재는 Seq 1(절단) 부터 시작
> - **공정 Seq 는 결정론적** — 동일 `PRODUCT.model_code` 는 동일 Seq 목록 생성. `BOM_RULE` 로 선택 공정 추가/제거 가능 (verb=ADD/REMOVE target=MBOM_PROCESS)
> - **Q19(b) 공정별 CT(표준 작업시간) 은 원천 미관리** — 본 문서에서도 필수 필드로 취급하지 않음. Phase 2 MES 실적 역산 대상

> 공정 마스터 카탈로그는 [[DE22-1_화면설계서/sections/03_공정관리]] SCR-PM-007 및 [[DE22-1_화면설계서/sections/07_공통CM]] SCR-CM-006 `PRC_TYPE` 코드그룹과 1:1 대응한다.
...
- **결정표 충돌·공백 계산**: 규칙 수 ≤100 은 FE AST 교집합 연산, >100 은 서버 `GET /api/pm/products/{productCode}/rules/decision-table` incremental 모드

---

## 9. 엔티티 스키마 요약 (용어사전 v1.4 기준)

본 절은 DE35-1 에서 참조·운용하는 BOM 엔티티의 신규·확장 속성을 정리한다. 물리 스키마 확정은 [[DE32-1_BOM도메인_ER다이어그램_v1.2]] 소관.
...
미서기 계열별 후렘 절단 보정 상수. 동일 Rule 구조에서 `when_expr` 의 `series_code` 분기로 격납된다 ([[WIMS_용어사전_BOM_v1.4]] §13.1).
...
[[WIMS_용어사전_BOM_v1.4]] §7 금지어 전수 점검:
...
| **v1.5** | **2026-04-16** | **코드크래프트** | **용어사전 v1.3 반영 — 단독 SOT 재구성. 1) PRODUCT(`series_code`/`class_l1~4`/`derivative_of/kind`/`dies_revision`), ITEM(`item_category`/`dies_code`/`unit_weight_kg_m`), OPTION_VALUE(`value_type`/`numeric_min/max`/`unit`/`enablement_condition`), MBOM(절단 9속성), BOM_RULE(`rule_type`/`action_json` verb 4종), RESOLVED_BOM(`rule_engine_version`) 신규/확장 (§3.6, §9) 2) 4계층 제품분류 통합 ER (§3.2~3.3) 3) `CuttingBOM` 엔티티 폐기 — MBOM 속성 확장으로 대체 (§3.6) 4) BOM_RULE 액션 동사 스키마 `SET`/`REPLACE`/`ADD`/`REMOVE` 공식화, 미서기 Rule 카탈로그 10건 (§6.5) 5) DHS-AE225-D-1 실 데이터 샘플 INSERT/MBOM/RESOLVED 행 (§10) 6) 계열별 절단 상수 차이표 (§11) 7) 다이스북·공급망 엔티티 3종 (§9.7) 8) 금지어(`CuttingBOM`·`산식구분`·`계산식`·`공식`) 본문 0건 달성 (§12.1) 9) [[DE11-1_소프트웨어_아키텍처_설계서_v1.2]] ER 보강본, `DE24-1 v1.7` 과 정합** |
| v1.5-r2 | 2026-04-16 | 코드크래프트 | BOM-RULE-UI 스펙 (v1-r1, 2026-04-16, archived) 반영. §6.5.2 `BOM_RULE` +5컬럼(template_id·template_instance_id·slot_values·scope_type·estimate_id), §6.5.3 `RULE_TEMPLATE` 신설 + 빌트인 6종, §6.5.4 `BOM_RULE_HISTORY` 신설, §6.5.5 템플릿 컴파일 규칙. §8.2 인덱스 +3. 참조: 용어사전 v1.4 §13 |
| **v1.6** | **2026-04-24** | **코드크래프트** | **유니크시스템 BOM 2026-04-24 보완본 반영. BOM Level 5단 공식 정의(L1 FERT / L2 HALB / L3 Sub-Asm / L4 Part / L5 ROH, §3.2.1), 위치구분 10종 카탈로그(H01~H03-2·W01~W03 + 품목별 사용 패턴·빈도, §6.4, Q16 해소), 제품 카탈로그 25종(후렘18·문짝6·방충망1, §4A, Gain G), 공정 순서 표준(후렘13·문짝12·방충망6, §7A, Q19(a) 해소). Q9 Phantom 해소 콜아웃(§3.4) 갱신. 원천: [[7_유니크시스템BOM_26.04.24_분석]] §4 Gain A·B·C·G.** |
...
- [[WIMS_용어사전_BOM_v1.4]]
- [[7_유니크시스템BOM_26.04.24_분석]] — **v1.6 원천**
- [[DE11-1_소프트웨어_아키텍처_설계서_v1.2]]
- [[DE32-1_BOM도메인_ER다이어그램_v1.2]]
- [[DE24-1_인터페이스설계서_v2.0]]
- [[GAP_분석_통합_2026-04-15]]
