...
**문서코드:** DE33-1
**버전:** v1.3
**작성일:** 2026-04-21 (v1.0) / 2026-04-22 (v1.1 → v1.2) / 2026-04-27 (v1.3)
**작성자:** 김지광 (PM, 코드크래프트)
**검토자:** 김진호 (BE, 코드크래프트)
**상태:** 확정판 (BE 구현 착수용)

> [!abstract] 문서 개요
> 본 문서는 DE32-1(논리 ER) 과 용어사전 v1.4 를 기반으로 **MariaDB 10.11 에서 실행 가능한 물리 DDL** 과 Flyway 마이그레이션 계획을 정의한다. BE 팀의 `wims-rule-engine` / 인증 모듈 구현 착수 전 스키마 확정 입력물.
>
> 범위: BOM 도메인 PM 서브시스템 19 엔티티 + 공급망 확장 2 엔티티 + 프로젝트 즐겨찾기 1 엔티티 + **CM 인증 도메인 2 엔티티 (admin · admin_login_history)** + 인덱스·FK·트리거 + 시드 데이터(옵션 그룹 7종·빌트인 규칙 템플릿 6종).
>
> **v1.3 (2026-04-27):** **CM 인증 도메인 2 엔티티 신설** — §3.21 `admin` (관리자 = 일반 사용자 단일 모델, IF-CM-AUTH-001/002/003 백엔드 SOT) + §3.22 `admin_login_history` (로그인 이력 감사). status enum `JOINED` / `SUSPENDED` / `WITHDRAWN`, bcrypt 해시 + DEC-03 10자 정책, 5회 실패 시 `SUSPENDED` 자동 전환 + `IF-EVT-USR-001(USER_LOCKED)` 발행. Flyway **V107 · V108** 추가. **엔티티 23→25, FK 31→32 (Phase 1 활성 29→30), 인덱스 48→52.** 개방이슈 #4 (PRODUCT↔DIES_BOOK FK 방향) 보류 유지.
>
...
1. **DE32-1 논리 모델과 1:1 대응.** 엔티티 추가·삭제 없음. 컬럼 추가는 물리 레벨 필수(감사·타임스탬프·버전) 에 한함. **예외:** v1.3 `admin` / `admin_login_history` 는 CM 인증 도메인으로 BOM ER 범위 밖이지만, IF-CM-AUTH-001/002/003 백엔드 SOT 확정에 따라 본 물리 스키마에 포함한다 (DE32-1 은 BOM 도메인 한정으로 대응 엔티티 없음).
...
3. **애플리케이션 레벨 규칙은 DDL 에 반영하지 않음.** frozen 불변성 중 APP 레벨 체크는 JPA `@PreUpdate` (DE11-1 ADR-006). DDL 은 **보호선 트리거** 로 최소한의 이중 방어만 수행. 5회 실패 시 `admin.status='SUSPENDED'` 자동 전환 / `IF-EVT-USR-001` 발행 / Redis `loginfail:{adminId}` 카운터 등 인증 정책은 **APP 계층** (Spring Security `AuthenticationFailureHandler`) 에서 처리하며 DDL 에는 반영하지 않는다.
4. **Phase 2 엔티티 참조 보류.** `BOM_RULE.estimate_id` 는 컬럼만 정의하고 FK 는 `ESTIMATE` 테이블이 Phase 2 에서 추가될 때 ALTER 로 부여.
5. **관리자 = 일반 사용자 단일 모델.** Phase 1 에서는 별도 일반 사용자 테이블 없이 `admin` 단일 엔티티가 IF-CM-AUTH-001/002/003 의 백엔드 SOT 역할을 한다. ROLE 분기는 추후 (DE11-1 §7.3) 추가되며 현 시점 `admin` 본체에는 ROLE 컬럼을 두지 않는다 (Phase 2 사용자/역할 정규화 시 별도 테이블 분리).
...
| 감사 (ADMIN_LOGIN_HISTORY → ADMIN) | `RESTRICT` | `CASCADE` | 로그인 이력은 관리자 탈퇴(WITHDRAWN) 후에도 보존. 하드 삭제 금지 |
...
- `ADMIN`: `status='WITHDRAWN'` 으로 탈퇴 처리 (하드 삭제 금지). `admin_login_history` FK `RESTRICT` 와 정합. 로그인 ID(`admin_id`) 는 탈퇴 후에도 재사용 금지(UNIQUE 제약 유지). 5회 실패 잠금은 `status='SUSPENDED'` 별도 상태로 처리.
...
>
> **v1.3 범위:** §3.1~§3.16 은 Phase 1 BOM 도메인 19 엔티티 중 16개(나머지 3개는 §3.17~§3.19 공급망 확장). §3.17~§3.19 는 v1.1 추가 공급망 확장 2엔티티 + 공급사 이력 1엔티티. **§3.20 `project_favorite` 는 v1.2 신설** (DE22-1 v1.6 프로젝트 관심 저장 이슈 해결). **§3.21 `admin` / §3.22 `admin_login_history` 는 v1.3 신설** (CM 인증 도메인, IF-CM-AUTH-001/002/003 백엔드 SOT).
...
### 3.21 admin *(v1.3 신설, Flyway V107)*

WIMS 2.0 Phase 1 의 **관리자 = 일반 사용자 단일 모델**. IF-CM-AUTH-001/002/003 (`POST /api/auth/login` · `/refresh` · `/logout`, [[DE24-1_인터페이스설계서_v2.0]] §5.2.1) 의 백엔드 SOT. JWT (HS256) access 1h / refresh 24h, refreshToken 은 Redis 보관. 비밀번호는 bcrypt 해시 + DEC-03 10자 정책. 5회 실패 시 `status='SUSPENDED'` 자동 전환 + `IF-EVT-USR-001(USER_LOCKED)` 발행 (APP 계층).

```sql
CREATE TABLE admin (
  id              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  admin_id        VARCHAR(64)     NOT NULL                COMMENT '로그인 ID. 탈퇴 후에도 재사용 금지',
  password        VARCHAR(255)    NOT NULL                COMMENT 'bcrypt 해시 (cost=10 권장)',
  name            VARCHAR(64)     NOT NULL,
  mobile          VARCHAR(20)     NULL,
  email           VARCHAR(255)    NULL,
  status          VARCHAR(16)     NOT NULL DEFAULT 'JOINED' COMMENT 'JOINED(가입) | SUSPENDED(제재) | WITHDRAWN(탈퇴)',
  latest_at       DATETIME(3)     NULL                    COMMENT '마지막 로그인 시각',
  created_at      DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  created_by      VARCHAR(64)     NOT NULL,
  updated_at      DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  updated_by      VARCHAR(64)     NOT NULL,
  PRIMARY KEY (id),
  UNIQUE KEY uq_admin_admin_id (admin_id),
  UNIQUE KEY uq_admin_email (email),
  KEY idx_admin_status (status),
  CONSTRAINT chk_admin_status CHECK (status IN ('JOINED','SUSPENDED','WITHDRAWN'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```

> **운영 정책:**
> - `password` bcrypt cost 는 BE 환경설정. 평문 저장 금지 (체크리스트 §8.1).
> - `email` UNIQUE 는 NULL 다중 허용 (MariaDB 기본 동작). 비밀번호 재설정 메일(IF-EXT-EMAIL-001) 동작을 위해 강력 권장.
> - `status='SUSPENDED'` 자동 전환 트리거는 Redis `loginfail:{adminId}` 카운터가 5 도달 시 APP 계층에서 수행. 잠금 해제는 ADMIN 의 수동 `PATCH .../unlock` 또는 비밀번호 재설정 성공 시.
> - `latest_at` 은 로그인 성공 시마다 갱신. 이력 상세는 `admin_login_history` 참조.
> - ROLE 컬럼 미보유: Phase 1 단일 사용자 모델로 모든 admin 은 동일 권한. ROLE 정규화는 후속 (DE11-1 §7.3 ROLE 사전 / Phase 2).
> - JWT secret 키는 prod 에서 환경변수 `JWT_SECRET` 강제, dev 는 `application-local.yml` fallback ([[DE24-1_인터페이스설계서_v2.0]] §3.2.1).

### 3.22 admin_login_history *(v1.3 신설, Flyway V108)*

`admin` 로그인 시도 이력 (성공/실패 양쪽). FR-CM-001 인증 감사 + NFR-MT-CM-002 변경 이력 감사 추적. IPv6 대응 `VARCHAR(45)`. 5회 실패 잠금 추적 시 `success=FALSE` 행을 시간순으로 카운트 (Redis 카운터의 영속 백업 역할).

```sql
CREATE TABLE admin_login_history (
  id              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  admin_id        BIGINT UNSIGNED NOT NULL                COMMENT 'admin.id (FK)',
  login_at        DATETIME(3)     NOT NULL                COMMENT '시도 시각',
  ip              VARCHAR(45)     NULL                    COMMENT 'IPv4/IPv6 주소',
  user_agent      VARCHAR(500)    NULL,
  success         BOOLEAN         NOT NULL,
  fail_reason     VARCHAR(64)     NULL                    COMMENT 'INVALID_PASSWORD | USER_NOT_FOUND | USER_SUSPENDED | USER_WITHDRAWN | OTHER',
  created_at      DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (id),
  KEY ix_admin_login_history_admin_id_login_at (admin_id, login_at DESC),
  KEY ix_admin_login_history_success (success, login_at DESC),
  CONSTRAINT fk_admin_login_history_admin FOREIGN KEY (admin_id) REFERENCES admin (id) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT chk_admin_login_history_fail_reason CHECK (fail_reason IS NULL OR fail_reason IN ('INVALID_PASSWORD','USER_NOT_FOUND','USER_SUSPENDED','USER_WITHDRAWN','OTHER'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```

> **감사 컬럼 예외:** 본 테이블은 자기 자신이 감사 테이블이므로 `created_at` 만 보유 (`updated_at`/`created_by`/`updated_by` 미보유, §2.3 `BOM_RULE_HISTORY` 와 동일 정책).
>
> **보존 기간:** NFR-SC-CM-006 감사로그 90일 이상. 운영 1년차 이후 파티셔닝 검토 (개방이슈 #11).
>
> **연관 흐름:**
> - 로그인 성공 → INSERT 1행(`success=TRUE`) + `admin.latest_at` UPDATE + Redis `loginfail:{adminId}` 0 으로 reset
> - 로그인 실패 → INSERT 1행(`success=FALSE, fail_reason=...`) + Redis 카운터 +1 → 5 도달 시 APP 이 `admin.status='SUSPENDED'` UPDATE + `IF-EVT-USR-001` 발행
...
v1.0: 인덱스 43건. **v1.1 추가 6건** (아래 § 끝 "v1.1 추가" 행) → v1.1 총 45건. **v1.2 추가 3건** (`uk_dies_book_series_rev`, `idx_pfav_user_time`, `idx_pfav_project`, `idx_ish_item_code_time`; 단 `idx_item_supplier_valid` → `idx_item_supplier_effective` 는 rename 이므로 신규 카운트 제외) → v1.2 총 48건. **v1.3 추가 4건** (`uq_admin_admin_id`, `uq_admin_email`, `idx_admin_status`, `ix_admin_login_history_admin_id_login_at`, `ix_admin_login_history_success` ※ admin PK · admin_login_history PK 는 PK 카운트로 +2 분리) → 총 **인덱스 52건** (PK 25 + UK 16 + 보조 13).
...
| **v1.3 추가** | | | | | |
| 49 | admin | uq_admin_admin_id | admin_id | UNIQUE | 로그인 ID 유일·재사용 금지 |
| 50 | admin | uq_admin_email | email | UNIQUE | 비밀번호 재설정 메일 식별 |
| 51 | admin | idx_admin_status | status | 단일 | SUSPENDED 자동 전환 모니터링 |
| 52 | admin_login_history | ix_admin_login_history_admin_id_login_at | (admin_id, login_at DESC) | 복합 | 사용자별 시도 이력 시간순 |
| (52+) | admin_login_history | ix_admin_login_history_success | (success, login_at DESC) | 복합 | 실패 5회 카운트 백업 조회 |

### 4.2 FK 관계 요약 (v1.3: 32건 중 Phase 1 활성 30건, Phase 2 ALTER 2건 보류)
...
| +v1.3-a | admin_login_history.admin_id | admin.id | RESTRICT | CASCADE |
...
- [ ] DE32-1 의 19 엔티티 + v1.1 공급망 확장 2 (dies_book_revision, item_supplier_history) + v1.2 신설 1 (project_favorite) + **v1.3 신설 2 (admin, admin_login_history)** = **총 25 엔티티** 모두 CREATE TABLE 로 정의됨 (§3.1~3.22)
- [ ] DE32-1 §3.1 FK 27건 + 추가 4건(+1/+2/+v1.1-a/+v1.1-b) + v1.2 2건(project_favorite Phase 2 ALTER) + v1.3 1건(admin_login_history → admin) = **총 32건 FK** 중 Phase 1 활성 30건 (§4.2)
- [ ] DE32-1 §5 인덱스 12건 + 추가 보조 인덱스 포함 (§4.1, v1.3 기준 **총 52건**)
- [ ] Flyway V100~V108 파일명 규칙 `V{n}__snake_case.sql` 준수 확인 (§7.1)
- [ ] 용어사전 v1.4 금지어(§7) 미사용 — `cutting_bom`/`layout_type`/`product_series`/`formula_kind`/`산식구분` grep 결과 0
- [ ] 모든 enum 컬럼에 CHECK 제약 (`role IN (PRIMARY/SECONDARY/EMERGENCY)`, `status IN ('JOINED','SUSPENDED','WITHDRAWN')`, `fail_reason IN (...)` 포함)
- [ ] 모든 테이블에 감사 컬럼 4종 (created_at/by, updated_at/by) — BOM_RULE_HISTORY, ITEM_SUPPLIER_HISTORY, ADMIN_LOGIN_HISTORY 제외 (자체 감사 테이블)
- [ ] **CM 인증 정합 (v1.3 신규)**: `admin.password` 평문 저장 금지 (bcrypt 해시 검증), `admin.status` 초기값 `JOINED`, IF-CM-AUTH-001/002/003 의 백엔드 구현이 본 테이블에 매핑되었는지 (DE24-1 §5.2.1 정합)
...
| **11** *(v1.3 신규)* | **`admin_login_history` 파티셔닝 — 운영 1년차에 대량 누적 시 연 단위 `login_at` 파티셔닝 vs 단일 테이블 결정.** NFR-SC-CM-006 90일 보관과 정합. | open | 운영 1년차 | DBA |
| **12** *(v1.3 신규)* | **`admin` ↔ `wims_user` 정규화 — Phase 2 에서 일반 사용자/역할 분리 시 admin 을 별도 ROLE 행으로 흡수할지, 별도 테이블 유지할지 (DEC 후속). 마이그레이션 시 `admin.id` 보존 필요.** | open | Phase 2 사용자 모델 확정 시 | PM + BE |
