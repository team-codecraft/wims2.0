---
title: DE24-1 인터페이스 설계서 v2.0 (통합본)
doc_code: DE24-1
version: 2.0
created: 2026-04-07
updated: 2026-04-23
...
**문서코드:** DE24-1
**버전:** v2.0
**작성일:** 2026-04-07 (최종 개정 2026-04-23)
**작성자:** 김진호 (BE, 코드크래프트)
**검토자:** 김지광 (PM, 코드크래프트)
...
| [[DE33-1_DB물리스키마_설계서_v1.2]] | DB 물리 스키마 | 테이블·Flyway·데이터 인터페이스 |
| [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.5]] | 표준 BOM 구조 | BOM 계층·품목코드 체계 |
...
| accessToken TTL | **2시간 (7,200초)** |
| refreshToken TTL | **14일** (기본) / 30일 (ADMIN 설정 시) |
| 전송 | `Authorization: Bearer <token>` |
| 서명 키 보관 | AWS Parameter Store (`/wims/jwt/secret`) |

내부 FE 사용. TTL 은 [[SCR-CM-007_시스템설정]] 세션·잠금 카테고리에서 ADMIN 이 조정 가능.
...
MES 서비스 계정은 `uid`·`grp` 미포함, `sub`=`mes-service`, `role`=`ROLE_MES_READER`.
...
| 인증 | 사용자 JWT (accessToken TTL 2시간) |
| 응답 래퍼 | §3.4 재사용 |
...
| Method | Path | 설명 | 권한 |
|--------|------|------|-----|
| POST | `/api/auth/login` | 로그인 (loginId + password → accessToken/refreshToken) | 비인증 |
| POST | `/api/auth/refresh` | 토큰 갱신 | 비인증 (refreshToken 필수) |
| POST | `/api/auth/logout` | 로그아웃 (refreshToken 블랙리스트 등록) | `ROLE_PM_VIEWER` |
| POST | `/api/auth/password-reset-request` | 비밀번호 재설정 이메일 발송 | 비인증 |
| POST | `/api/auth/password-reset-confirm` | 토큰 검증 + 신규 비밀번호 설정 | 비인증 (reset token) |

**Request (login):**

```json
{ "loginId": "kim.jinho", "password": "********" }
```
...
| 403 | `USER_LOCKED` | 5회 실패 잠금 (SCR-CM-007 시스템설정) |
| 422 | `USER_PASSWORD_POLICY_VIOLATION` | 최초 로그인 시 비밀번호 변경 필요 |
...
**데이터 근거**: [[DE33-1_DB물리스키마_설계서_v1.2]] §3.15 `BOM_RULE_HISTORY` 테이블. `(rule_id, changed_at DESC)` 인덱스 활용.
...
적용 범위 `scopeType=ESTIMATE` 로 저장되며, 표준 BOM 본체에는 영향 없음 ([[DE33-1_DB물리스키마_설계서_v1.2]] `BOM_RULE_OVERLAY` 참조).
...
| POST | `/api/pm/projects/{projectNo}/favorite` | 즐겨찾기 추가 ([[DE33-1_DB물리스키마_설계서_v1.2]] V106) | `ROLE_PM_VIEWER` |
...
- [[DE33-1_DB물리스키마_설계서_v1.2]] — 테이블·Flyway V1~V106
- [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.5]] — BOM 계층·품목코드
