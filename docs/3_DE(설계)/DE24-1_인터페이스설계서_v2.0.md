---
title: DE24-1 인터페이스 설계서 v2.0 (통합본, r4)
doc_code: DE24-1
version: 2.0-r4
created: 2026-04-07
updated: 2026-04-27
...
# DE24-1 인터페이스 설계서 v2.0 (통합본, r4)

**문서코드:** DE24-1
**버전:** v2.0-r4 (내부 revision, v2.0 본문 보존)
...
| 버전 | 일자 | 작성자 | 변경 내용 |
|------|------|--------|----------|
| **v2.0-r4** | **2026-04-27** | **김지광** | **IF-CM-USR-001 사용자(=관리자) 등록 본문 명세 보강.** (1) §5.2.3 본문에 `POST /api/cm/users` Request DTO (`CreateAdminRequest` — adminId/password/name/mobile/email) · Response DTO (`CreateAdminResponse` — id/adminId/name/email/mobile/status/createdAt) 표 추가. (2) `password` 필드는 IF-CM-AUTH-004 공개키로 RSA-OAEP-SHA256 암호화한 Base64 ciphertext 임을 명시 (평문 전송 금지, 로그인 흐름과 동일 일관성). (3) 검증 규칙 (10자 정책·adminId UNIQUE·email UNIQUE) 명문화. (4) 에러 카탈로그에 `ADMIN_ID_DUPLICATE` (409) · `EMAIL_DUPLICATE` (409) · `INVALID_PASSWORD_CIPHER` (400) · `PASSWORD_POLICY` (400) 추가. (5) 첫 admin 부트스트랩은 DE33-1 §6.4 / Flyway V109 시드 마이그레이션에 위임. **메타: r4 = v2.0 본문 보존 + internal revision** (파일명 v2.0 유지). |
| **v2.0-r3** | **2026-04-27** | **김지광** | **로그인 비밀번호 전송 구간 RSA-OAEP-SHA256 암호화 도입.** (1) §3.2.6 신설 — RSA-2048 / OAEP-SHA-256 + MGF1-SHA-256 / X.509 DER Base64 공개키 / 서버 시작 시 메모리 생성·재시작 회전 정책 / 클라이언트 가이드 (JS / Java / Kotlin 예시). (2) §5.2.1 IF-CM-AUTH-001 — `LoginRequest.password` 필드 정의를 평문→`Base64(RSA-OAEP-SHA256(plaintext))` 로 갱신. 에러 카탈로그에 `INVALID_PASSWORD_CIPHER` (400) · `PASSWORD_POLICY` (400) 추가. (3) §5.2.1 IF-CM-AUTH-004 신설 — `GET /api/auth/public-key` (permitAll). (4) §8 IF-ID 매트릭스에 IF-CM-AUTH-004 추가. **메타: r3 = v2.0 본문 보존 + internal revision** (파일명 v2.0 유지). |
...
#### 3.2.6 로그인 비밀번호 전송 암호화 (v2.0-r3)

> [!info] 도입 배경 (2026-04-27)
> TLS (NFR-SC-CM-003) 만으로도 회선 도청은 차단되지만, **TLS 종단 (LB / WAF / 프록시) 이후의 평문 노출**, 디버그 캡처, 로그 미스매스크 등 운영 단계의 우발적 노출 면을 줄이기 위해 `/api/auth/login` 의 `password` 필드를 **응용 계층에서 한 번 더 암호화**한다 (TLS 위 이중 방어, defense-in-depth).

**알고리즘·키 정책:**

| 항목 | 규격 |
|------|------|
| 알고리즘 | **RSA-OAEP** (RSA / ECB / OAEP padding) |
| 해시 / MGF | **SHA-256** + MGF1-SHA-256 (`OAEPParameterSpec(SHA-256, MGF1, MGF1ParameterSpec.SHA256, PSpecified.DEFAULT)`) |
| 키 길이 | **RSA-2048** |
| 키 생성 | 서버 부팅 시 `KeyPairGenerator("RSA")` 로 메모리 생성 |
| 키 보관 | **메모리만** (디스크 / DB / Vault 저장 X) |
| 키 회전 | **재시작 시 자동 회전** (의도적 회전은 인스턴스 재기동) |
| 공개키 인코딩 | **X.509 (SubjectPublicKeyInfo) DER → Base64** (PEM header/footer 없음) |
| ciphertext 인코딩 | RSA-OAEP 출력 256 bytes → **Base64** |

**공개키 발급 — IF-CM-AUTH-004:**
...
**로그인 흐름 갱신 (IF-CM-AUTH-001):**

1. 클라이언트 → `GET /api/auth/public-key` 호출 (페이지 진입 시 또는 로그인 직전 1회).
2. 받은 X.509 DER Base64 공개키로 평문 password 를 RSA-OAEP-SHA256 암호화 → Base64 인코딩.
3. `POST /api/auth/login` 호출 시 `password` 필드에 **Base64(ciphertext)** 를 그대로 전송.
4. 서버 [`AdminAuthService.login`](#5-2-1-인증-scr-cm-001-로그인) → `RsaKeyService.decrypt(cipher)` 로 평문 복원.
   - 복호화 실패 → `400 INVALID_PASSWORD_CIPHER` (Base64 손상·키 불일치·padding 오류 모두 동일 코드).
   - 복호화 결과 길이가 10자 미만 → `400 PASSWORD_POLICY` (DEC-03).
   - 그 외 정상 → 기존 bcrypt 비교 흐름 (`AuthError.INVALID_CREDENTIALS` / 5회 SUSPENDED 등).

> [!warning] 키 회전 / 공개키 캐시
> 클라이언트가 공개키를 장기 캐시할 경우 서버 재시작 후 첫 로그인이 `INVALID_PASSWORD_CIPHER` 로 실패할 수 있다. SPA 는 **로그인 페이지 mount 시 매번 재발급** 하거나, 401·400 발생 시 1회 자동 재발급 + 재시도 패턴을 권장.

**클라이언트 예시 (Web Crypto API, JavaScript):**

```javascript
// 1) 공개키 획득
const { publicKey } = await fetch('/api/auth/public-key').then(r => r.json());

// 2) X.509 DER Base64 → ArrayBuffer → CryptoKey
...
// 3) plaintext password 암호화 → Base64
const ct = await crypto.subtle.encrypt(
  { name: 'RSA-OAEP' }, key,
  new TextEncoder().encode(plainPassword)
);
const password = btoa(String.fromCharCode(...new Uint8Array(ct)));
...
val password = Base64.getEncoder().encodeToString(cipher.doFinal(plainPassword.toByteArray(Charsets.UTF_8)))
```

> [!info] 적용 범위
> 본 절은 **`/api/auth/login` 의 `password` 필드 한정**. `/api/cm/me/password` (비밀번호 변경, §5.2.2) 등 다른 엔드포인트의 비밀번호 필드 적용은 후속 (PR 범위 외). MES 서비스 계정 (§3.2.2) 의 `clientSecret` 은 별도 정책.
...
|-------|--------|------|------|-----|
| **IF-CM-AUTH-001** | POST | `/api/auth/login` | 관리자 로그인 (admin_id + **Base64(RSA-OAEP-SHA256(password))** → access/refresh) | 비인증 |
...
| **IF-CM-AUTH-004** | GET | `/api/auth/public-key` | 비밀번호 전송 암호화용 공개키 발급 (X.509 DER Base64, §3.2.6) | 비인증 |
...
| `password` | string | ✓ | **Base64(RSA-OAEP-SHA256(plaintext))** — IF-CM-AUTH-004 로 발급받은 공개키로 암호화 (DE24-1 §3.2.6). 서버가 복호화 후 DEC-03 10자 정책 + bcrypt 비교. |

```json
{ "adminId": "admin01", "password": "OZ5xYm1...Base64-RSA-OAEP-ciphertext..." }
```

> [!warning] 평문 password 전송 금지
> v2.0-r3 부터 평문 password 를 그대로 보내면 `400 INVALID_PASSWORD_CIPHER` 로 실패한다. 클라이언트는 반드시 IF-CM-AUTH-004 의 공개키로 RSA-OAEP-SHA256 암호화 후 Base64 로 인코딩해 전송해야 한다 (§3.2.6 클라이언트 가이드 참조).
...
| 400 | `INVALID_PASSWORD_CIPHER` | `password` 가 RSA-OAEP-SHA256 ciphertext 가 아니거나 Base64/패딩 손상·키 불일치(공개키 회전 후 캐시 사용 등). v2.0-r3 신설 (§3.2.6) |
| 400 | `PASSWORD_POLICY` | 복호화된 평문 비밀번호가 DEC-03 10자 정책 미달. v2.0-r3 신설 |
...
##### IF-CM-AUTH-004 공개키 발급 (v2.0-r3 신설)

비밀번호 전송 구간 RSA-OAEP-SHA256 암호화용 공개키 발급. 정책 상세는 [§3.2.6](#3-2-6-로그인-비밀번호-전송-암호화-v2-0-r3) 참조.

**Request:**

```
GET /api/auth/public-key
Auth: 비인증 (`/api/auth/**` permitAll)
```

**Response (200, `PublicKeyResponse`):**

| 필드 | 타입 | 설명 |
|------|------|------|
| `publicKey` | string | **X.509 (SubjectPublicKeyInfo) DER → Base64** (PEM header/footer 없음) |
| `algorithm` | string | `"RSA-OAEP-SHA256"` 고정 |
| `keyFormat` | string | `"X.509"` 고정 |
| `keySize` | int | `2048` 고정 |

```json
{
  "publicKey": "MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA...AQAB",
  "algorithm": "RSA-OAEP-SHA256",
  "keyFormat": "X.509",
  "keySize": 2048
}
...
- 키 쌍은 서버 부팅 시 [`RsaKeyService`](#3-2-6-로그인-비밀번호-전송-암호화-v2-0-r3) 가 메모리 생성. 디스크 저장 X.
- 인스턴스 재시작 = 자동 회전. 다중 인스턴스 환경에서는 인스턴스별로 다른 공개키를 보유 가능 → 클라이언트는 **로그인 페이지 mount 시 매번 재발급** 권장 (sticky session 미적용 환경 대응).
- 클라이언트가 stale 공개키로 암호화 시 → `400 INVALID_PASSWORD_CIPHER` → 자동 재발급 후 재시도 패턴 권장.

**Errors:** 본 엔드포인트는 키 가용 상태에서 항상 `200 OK` 를 반환 (입력 파라미터 없음). 키 생성 실패는 부팅 시점에 fail-fast.
...
##### IF-CM-USR-001 사용자 등록 (POST /api/cm/users) (v2.0-r4)

WIMS 2.0 단일 사용자 모델 (사용자 = 관리자) 의 신규 등록. 인증된 admin (Phase 1 모든 admin) 만 호출 가능 — `SecurityConfig` 가 `/api/cm/**` 를 authenticated 처리한다. 첫 admin 은 시드 마이그레이션(DE33-1 §6.4 / Flyway V109) 으로 부트스트랩한다.

**Request DTO (`CreateAdminRequest`)**:

| 필드 | 타입 | 필수 | 제약 | 설명 |
|------|------|------|-----|------|
| `adminId` | string | Y | 4–64자, `uq_admin_admin_id` UNIQUE | 로그인 ID. 탈퇴 후에도 재사용 금지 (DE33-1 §3.21). |
| `password` | string | Y | 1–1024자 (ciphertext 길이) | **IF-CM-AUTH-004 공개키로 암호화한 RSA-OAEP-SHA256 ciphertext (Base64).** 평문 전송 금지. 서버에서 복호화 후 평문 길이가 10자 이상인지 정책 검증 후 bcrypt(cost=10) 해시. |
| `name` | string | Y | 1–64자 | 표시명. |
| `mobile` | string | N | ≤20자 | 휴대전화 (선택). |
| `email` | string | N | ≤255자, `@Email`, `uq_admin_email` UNIQUE | 이메일 (선택). |

**Response 201 Created (`CreateAdminResponse`)**:
...
비밀번호 해시·감사 컬럼은 응답에 노출하지 않는다.

**검증 규칙**:

- **암호화**: `password` 가 IF-CM-AUTH-004 공개키로 RSA-OAEP-SHA256 암호화된 Base64 ciphertext 가 아니면 400 `INVALID_PASSWORD_CIPHER`. (서버는 키 회전 정책 — 재시작마다 회전 — 으로 인해 만료 시 동일 에러 반환 → 클라이언트는 공개키 재발급 후 재시도.)
- **비밀번호 정책 (DEC-03 / NFR-SC-CM-004)**: 복호화한 평문이 **10자 미만**이면 400 `PASSWORD_POLICY`.
- **adminId 중복**: `findByAdminId` 사전 검사 + DB UNIQUE (`uq_admin_admin_id`) 동시성 보호 → 409 `ADMIN_ID_DUPLICATE`.
- **email 중복**: DB UNIQUE (`uq_admin_email`) 위반 시 409 `EMAIL_DUPLICATE`. NULL 허용.

**Errors**:

| HTTP | code | 의미 |
|------|------|------|
| 400 | `INVALID_PASSWORD_CIPHER` | 공개키 불일치·ciphertext 손상 등 RSA-OAEP 복호화 실패. 공개키 재발급 후 재시도. |
| 400 | `PASSWORD_POLICY` | 평문 비밀번호가 DEC-03 10자 정책 미달. |
| 400 | `VALIDATION_ERROR` | DTO Bean Validation 실패 (필수 누락, 길이 초과, email 형식 위반 등). |
| 401 | `INVALID_TOKEN` | 인증 토큰 누락/만료. (전역 처리.) |
| 409 | `ADMIN_ID_DUPLICATE` | `admin_id` 중복. |
| 409 | `EMAIL_DUPLICATE` | `email` 중복. |
...
| `IF-CM-AUTH-001` | 로그인 | 내부 REST | `POST /api/auth/login` | §5.2.1 | FR-CM-001, FR-CM-008 | SCR-CM-001 |
...
| `IF-CM-AUTH-004` | 비밀번호 암호화용 공개키 | 내부 REST | `GET /api/auth/public-key` | §3.2.6, §5.2.1 | FR-CM-008, NFR-SC-CM-003 | SCR-CM-001 |
