...
> [!info] MES 연동 상태 배지 실시간성 (v1.7 보강)
> - **폴링 주기**: 30초 (웹소켓 승격은 Phase 2 검토)
> - **상태**: 🟢 전달 중 (최근 성공) / 🟡 대기 (RELEASED 없음) / 🔴 장애 (최근 5회 실패)
> - **🔴 상태 시**: 클릭 시 재시도 모달 + 최근 에러 trace_id · 운영자 문의 CTA
> - **마지막 동기화 시각** 툴팁 표시 (예: "2분 전 동기화됨")
...
> [!info] Wizard ↔ Expert 모드 전환 규칙 (v1.7 보강)
> - **Wizard → Expert** (슬롯폼 → DSL 편집기): 항상 가능. 슬롯 값으로 `condition_expr` · `action_json` 자동 생성 후 Expert 뷰에 prefill
> - **Expert → Wizard**: 조건부. `condition_expr` 가 빌트인 템플릿 6종의 슬롯 표현으로 역변환 가능할 때만. 불가능 시 안내: "복합 조건식이라 Wizard 표현 불가 — Expert 유지"
> - 모드 전환 시 unsaved 변경 감지 → 확인 모달
...
> [!info] 409 Optimistic Locking · 동시 편집 merge helper (v1.7 신설)
> 3뷰(SCR-PM-021/022/023) 모두 동일 `BOM_RULE` 엔티티를 조작하므로 2인 이상 동시 편집 시 409 처리:
> - **상대 편집자 실시간 표시**: 화면 우상단 presence 뱃지 (예: "👤 김영희 편집 중"). 웹소켓 또는 polling 5초
> - **저장 시 If-Match / ETag**: 서버 200(성공) / 409(충돌)
> - **409 응답 시 merge helper 모달**:
>   - 3-way diff (내 변경 / 상대 변경 / 병합안)
>   - 3 선택: [내 변경 유지] / [상대 변경 덮어쓰기] / [병합 저장]
>   - 병합 시 필드 단위 cherry-pick 가능 (조건식·action·priority 개별)
> - **lock 대안**: 30초 이상 편집 중인 규칙은 soft lock 표시 (타 사용자가 알림받고 읽기전용 진입)
...
> [!warning] 확정(frozen) 더블체크 모달
> [확정] 버튼 클릭 시 다음 모달이 표시된다 (ADR-006 frozen 이중방어와 연동):
> - 제목: **"확정구성표를 생성하시겠습니까? (되돌릴 수 없음)"**
> - 본문: 영향 요약 — `resolvedBomId` 생성 예시 · MES 즉시 전달 여부 · snapshot 불변 컬럼 6종
> - **타이핑 확인**: 사용자가 `productCode` 를 직접 타이핑해야 [확정] 버튼 활성
> - 체크박스 2종: ☐ "옵션구성 검토 완료" · ☐ "MES 전달 준비 완료"
> - 버튼: [취소] / [확정 (되돌릴 수 없음)]
> - ESC 로 취소 가능, Enter 는 무시 (오타 방지)
...
> [!info] 결정표 셀 · 행 우클릭 메뉴 (5컬럼 매핑 활용)
> DEC-09 P1-17 에서 도입한 5컬럼(`template_id`·`template_instance_id`·`slot_values`·`scope_type`·`estimate_id`) 을 UI 에서 활용:
> - **셀 우클릭**: 규칙 편집·삭제·우선순위 변경·전문가 뷰 점프
> - **묶음 삭제 (template_instance_id 단위)**: "이 템플릿으로 생성된 N개 규칙 일괄 삭제" — 동일 `template_instance_id` 행 일괄 DELETE. 확인 모달 필수
> - **슬롯 값 편집**: `slot_values` JSON 을 Wizard 폼으로 재진입 (원본 보존, FR-PM-025 왕복 보존)
> - **scope 전환**: 셀 우클릭 "이 규칙을 ESTIMATE 오버레이로 전환" → 현재 견적 컨텍스트 선택 모달 (Phase 2 견적 진입 시 활성)
> - **행/열 우클릭**: 해당 옵션값의 모든 규칙 일괄 복제·삭제

> [!info] 우측 상세 패널 [이력] 탭 (v1.6-r1 신설, FR-PM-026 + FR-PM-021)
...
> [!info] AST 캐시 상태 뱃지 (v1.7 신설)
> 시뮬레이터 우상단에 평가 메타 표시:
> - **평가 시간**: 142ms
> - **캐시 상태**: 🟢 HIT (warm) / 🟡 MISS (first compile) / 🔴 COLD (bulk warm-up 필요)
> - **적용 규칙**: 4 / 12 (기각 8)
> - **rule_engine_version**: UNIQUE_V1
> - NFR-PF-PM-005: p95 < 200ms · NFR-MT-PM-001: warm-up ≤ 60s
