...
> - **DB 기준문서:** [[DE33-1_DB물리스키마_설계서_v1.2]] (MariaDB DDL). 각 화면 §DB 정합 체크리스트에서 gap 명시.
> - **프레임워크:** Vue 3 Composition API + Pinia + Vite ([[DE11-1_소프트웨어_아키텍처_설계서_v1.2]] §4). Form 은 VeeValidate + Zod 스키마.
> - **권한:** 조회 `ROLE_USER` / 편집 `ROLE_PRODUCT_EDITOR` (본 초안 표기). RuleEngine 권한 세트 `ROLE_PM_ADMIN/EDITOR/VIEWER` 와 통합은 §개방 이슈.
...
| 요소 | 상태 | 조건·동작 |
|---|---|---|
| `[+ 파생제품 추가]` 버튼 | enabled | `ROLE_PRODUCT_EDITOR` + 기본제품 `status='ACTIVE'` |
...
| Method | Path | 용도 | 인증 |
|---|---|---|---|
| GET | `/api/pm/products/{productCode}/derivatives` | 파생제품 목록 | ROLE_USER |
| POST | `/api/pm/products/{productCode}/derivatives` | 등록 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/products/{derivativeCode}` | 파생제품 수정 | ROLE_PRODUCT_EDITOR |
| DELETE | `/api/pm/products/{derivativeCode}` | 파생 삭제 (연결 RESOLVED_BOM 있으면 409) | ROLE_PRODUCT_EDITOR |
| GET | `/api/pm/bom-rules?ruleType=DERIVATIVE&productCode={code}` | 차이 규칙 조회 | ROLE_USER |

**권한별 UI 가시성**

| 역할 | `[+ 추가]` | 파생 상세 편집 | 차이 규칙 편집 | 삭제 |
|---|:---:|:---:|:---:|:---:|
| `ROLE_USER` | 숨김 | 읽기전용 | 읽기전용 | 숨김 |
| `ROLE_PRODUCT_EDITOR` | ✓ | ✓ | ✓ (DERIVATIVE 규칙만) | ✓ |
...
| `[+ 다이스북 등록]` | enabled | `ROLE_PRODUCT_EDITOR` |
| 행 클릭 | 상세 탭 전환 | 슬라이드 인 drawer (데스크톱) / 전체화면 (태블릿) |
| `[+ 개정판 발행]` | enabled | 상세 탭 진입 후 ROLE_PRODUCT_EDITOR |
...
| Method | Path | 용도 | 인증 |
|---|---|---|---|
| GET | `/api/pm/dies-books?q=&page=&size=` | 목록 (검색·페이지) | ROLE_USER |
| GET | `/api/pm/dies-books/{id}` | 상세 + 개정판 이력 | ROLE_USER |
| POST | `/api/pm/dies-books` | 등록 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/dies-books/{id}` | 수정 | ROLE_PRODUCT_EDITOR |
| POST | `/api/pm/dies-books/{id}/revisions` | 개정판 발행 | ROLE_PRODUCT_EDITOR |
| DELETE | `/api/pm/dies-books/{id}` | 삭제 (연결 product 없을 때만) | ROLE_PRODUCT_EDITOR |
...
| `[+ 공급사 등록]` | enabled | `ROLE_PRODUCT_EDITOR` |
...
| Method | Path | 용도 | 인증 |
|---|---|---|---|
| GET | `/api/pm/suppliers?role=&q=&active=&page=` | 목록 | ROLE_USER |
| GET | `/api/pm/suppliers/{id}` | 상세 (담당 자재 집계 포함) | ROLE_USER |
| POST | `/api/pm/suppliers` | 등록 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/suppliers/{id}` | 수정 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/suppliers/{id}/deactivate` | 비활성화 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/suppliers/{id}/activate` | 재활성화 | ROLE_PRODUCT_EDITOR |
...
| 요소 | 상태 | 조건·동작 |
|---|---|---|
| 우선순위 드래그 | enabled | `ROLE_PRODUCT_EDITOR`. 드래그 드롭 완료 후 자동 재번호 + 저장 확인 toast |
...
| Method | Path | 용도 | 인증 |
|---|---|---|---|
| GET | `/api/pm/items/{itemCode}/suppliers` | 자재별 공급사 목록 | ROLE_USER |
| POST | `/api/pm/items/{itemCode}/suppliers` | 매핑 추가 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/items/{itemCode}/suppliers/reorder` | 우선순위 일괄 재정렬 | ROLE_PRODUCT_EDITOR |
| PATCH | `/api/pm/items/{itemCode}/suppliers/{supplierId}` | 매핑 수정 (단가·리드타임·유효기간) | ROLE_PRODUCT_EDITOR |
| DELETE | `/api/pm/items/{itemCode}/suppliers/{supplierId}` | 매핑 삭제 | ROLE_PRODUCT_EDITOR |
...
| 6 | 권한 | `ROLE_PRODUCT_EDITOR` (04_제품관리) vs `ROLE_PM_EDITOR` (DE11-1) 명명 불일치 | 전체 | DE11-1 기준으로 통일: `ROLE_PM_ADMIN/EDITOR/VIEWER`. DE22-1 차기 개정에서 일괄 치환 | PM/BE | 중 |
...
- [[DE33-1_DB물리스키마_설계서_v1.2]] — DDL 정합 ground truth. §5.2 스키마 확장 요구
