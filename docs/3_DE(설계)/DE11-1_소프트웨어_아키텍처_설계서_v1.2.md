...
| [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.6\|DE35-1]] | BOM 구조 정의서 v1.5 | BOM 도메인 모델 |
...
> [!note] 본 문서에서 "모듈" 의 의미 (v1.3-r2 주석 추가)
> 본 문서에서 **모듈** 은 **논리적 단위** 를 의미하며, 물리적 Gradle 서브프로젝트 분리는 **구현 선택** 이다. 초기에는 **단일 Gradle 모듈 내 `com.codecraft.wims.ruleengine` 패키지** 로 구현하고, 패키지 경계 + Kotlin `internal` 가시성 + ArchUnit/detekt 규칙으로 외부(`pm`·`es`) 의 내부 구현 접근을 차단한다. 필요 시 별도 Gradle 서브프로젝트로 승격 가능(이때는 본 §11.1 "배포 형태" 행의 `implementation(project(":rule-engine"))` 이 활성). 단, ADR-005 대로 **마이크로서비스 분리는 금지**.

### 11.1 모듈 개요

| 항목 | 내용 |
|------|------|
| 모듈명 | `wims-rule-engine` (Kotlin 라이브러리 단위) |
| 구현 단위 | Phase 1 초기: `com.codecraft.wims.ruleengine` 패키지 (단일 Gradle 모듈). 필요 시 Gradle 서브프로젝트로 승격 |
| 버전 식별자 | `UNIQUE_V1` (`RESOLVED_BOM.rule_engine_version` 과 일치) |
| 배포 형태 | Spring Boot 애플리케이션에 embedded. 서브프로젝트 승격 시 `implementation(project(":rule-engine"))` 의존 |
...
- [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.6]] — BOM 도메인 모델
