...
type: 설계
status: review
supersedes: "[[DE11-1_소프트웨어_아키텍처_설계서_v1.1]]"
...
> [!abstract] v1.2 요약
> 용어사전 [[WIMS_용어사전_BOM_v1.3]] 에서 확정된 엔티티 확장·RuleEngine 모듈·산식 언어 `UNIQUE_V1`·snapshot 필드를 아키텍처에 반영한다. 주요 변경:
...
| **v1.3** | **2026.04.16** | **김지광** | **[[2026-04-16-bom-rule-ui-design]] 반영. §11.7 템플릿 컴파일러 명세(TemplateCompiler 인터페이스, 컴파일 규약 4종), §11.8 시뮬레이터 API(POST /pm/rules/simulate), §11.9 결정표 API(GET /pm/rules/decision-table). 근거 용어사전 v1.3→v1.4** |
...
본 문서는 WIMS 2.0 시스템의 소프트웨어 아키텍처를 정의한다. 5개 서브시스템(PM/ES/OM/MF/FS)과 공통 기능의 구조, 기술 스택, 레이어, 인프라, 배포 전략을 설계한다. v1.2 는 [[WIMS_용어사전_BOM_v1.3]] 확정 사항(RuleEngine, snapshot, 신규 엔티티)을 반영한다.
...
| 문서코드 | 문서명 | 용도 |
|---------|--------|------|
| [[WIMS_용어사전_BOM_v1.3\|용어사전 v1.3]] | BOM 도메인 용어사전 | **본 문서의 기준 — 엔티티·필드·RuleEngine 명세** |
| [[PP11-1_TFT구성계획서_v2.3\|PP11-1]] | TFT 구성계획서 v2.3 | 개발 환경, 브랜치 전략 |
| [[DE24-1_인터페이스설계서_MES_REST_API_v1.8\|DE24-1]] | 인터페이스 설계서 (MES REST API) v1.8 | API 아키텍처, 인증 |
| [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.5\|DE35-1]] | BOM 구조 정의서 v1.5 | BOM 도메인 모델 |
| [[V3_기존설계문서_영향도]] | 검증 V3 | DE11-1 영향 섹션 |
| [[V5_성능운영_리스크]] | 검증 V5 | 인덱스·AST 캐시·카디널리티 리스크 P2/P3/P4 |
...
AN12-1 NFR-PF 및 [[V5_성능운영_리스크]] P2·P3·P4 를 반영한 아키텍처 수준 SLA:
...
| 4 | MES API ≤ 2s | RESOLVED_BOM 조회 전용(재평가 없음) | 메모리 캐시 TTL 5분 | NFR-PF-PM-002 |
...
| 역할 | 코드 | 접근 범위 |
|------|------|----------|
| 최고관리자 | SUPER | 모든 기능 + 시스템 관리 |
| 프로젝트 멤버 | PR_MEM | 배정된 프로젝트 전체 기능 |
| 설계 멤버 | ARCH_MEM | 배정된 프로젝트 견적/도면 기능 |
| 일반 멤버 | MEM | 기본 조회 |
| MES 서비스 | ROLE_MES_READER | BOM/자재/공정 조회 전용 (API) — Spring Security ROLE_ 접두사 포함 |
...
| 대안 | (a) BOM_RULE 변경 시 전체 RESOLVED_BOM 일괄 재평가 — 분쟁 위험 증가로 기각. (b) frozen 없이 매번 재계산 — MES SLA(2s) 위반 및 데이터 무결성 상실 |
...
- [[2026-04-16-bom-rule-ui-design]] — BOM 옵션별규칙 관리 UI 설계 스펙 (§11.7~11.9 근거)
- [[DE24-1_인터페이스설계서_MES_REST_API_v1.8]] — MES 연동 엔드포인트
- [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.5]] — BOM 도메인 모델
- [[V3_기존설계문서_영향도]] — DE11-1 영향 분석
- [[V5_성능운영_리스크]] — P2/P3/P4 근거
- [[GAP_분석_통합_2026-04-15]] — DDL 총량
- [[DE11-1_소프트웨어_아키텍처_설계서_v1.1]] — 직전 버전 (원본 보존)
