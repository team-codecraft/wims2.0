...
> - **SOT 원칙:** 표기 충돌 시 [[WIMS_용어사전_BOM_v1.4]] 우선, 본 문서는 미서기이중창 도메인 적용본.
...
> 아래 표는 미서기 이중창(외측 2짝 + 내측 2짝) 1SET 기준의 표준 BOM 전개다. `Lv` 는 BOM 계층(숫자가 클수록 하위), `수량` 은 바로 위 조립체 1단위 기준 소요량이다. 용어가 낯선 경우 §2 · [[WIMS_용어사전_BOM_v1.4]] 을 참조한다.
