...
- **변경 이력 조회**: 모든 BOM_RULE CRUD 는 `BOM_RULE_HISTORY` ([[DE33-1_DB물리스키마_설계서_v1.3]] §3.15) 에 기록된다. 화면에서 `actor` / `changed_at` / `before_snapshot` / `after_snapshot` / `changed_fields` / `reason` 을 조회할 수 있어야 한다. 본 이력은 **90일 이상 보관** (NFR-SC-PM-002 감사 로그 90일).
...
- [[DE35-1_미서기이중창_표준BOM구조_정의서_v1.6]] — 표준 BOM 구조
