...
WIMS 2.0은 현행 시스템의 데이터를 계승하되, 구조적 한계를 근본적으로 개선한다. 전환 전략은 다음 3개 축으로 구성된다:

1. **데이터 모델 재설계**: BOM 계층 구조 도입, EBOM/MBOM 분리, 공정 마스터 신설
2. **데이터 품질 강화**: 자재코드 표준화, 유일성 검증, 구매/발주 시점 단가 이력 관리 (자재 마스터와 분리)
...
|--------|-------------|-------------|-----------|-------------|
| 자재·제품 | 플랫 자재 목록 + 수동 제품 매핑 | 4단계 BOM 계층 + EBOM/MBOM 분리 + 구성형 BOM | 자재코드 표준화, BOM 트리, 구매/발주 기반 단가 이력 (자재 마스터 분리), 공정 마스터 | DE32-1 ERD, DE35-1 BOM |
| 프로젝트 | 기본 CRUD + 파일 로컬 저장 | 프로젝트 중심 통합 관리 + S3 파일 저장 | 파일 S3 전환, 상태 워크플로우 강화 | DE32-1 ERD |
| 견적설계 | 도면 기반 수동 견적 | CAD 블록 자동인식 + BOM 기반 자동 산출 | BOM 연동 견적, 롤방충망 자동산출 | DE32-1 ERD |
| 계약·발주 | 수동 발주서 + 단순 절단 계획 | 최적 절단 알고리즘 + 자동 발주서 출력 | 절단 최적화, 발주서 자동화 | DE32-1 ERD |
| 인증·사용자 | 세션(Cookie) 기반 | JWT + Spring Security + 역할 기반 접근 제어 | JWT 토큰, RBAC 강화 | DE32-1 ERD |
| 제조관리 (신규) | 미구현 | 제작도 산출, 절단 지시서, MES 연동 | 전면 신규 개발, MES REST API | DE24-1, DE32-1 |
| 현장실측 (신규) | 미구현 | Android 앱 + BLE + 오프라인 동기화 | 모바일 전면 신규, BLE 연동 | DE32-1 ERD |
